...
def create_session_folder(base_dir: str | Path = Path("data") / "sessions", created_at: datetime | None = None) -> SessionInfo:
    ...
    return SessionInfo(session_id=session_dir.name, created_at=resolved_created_at, paths=paths)
...
def delete_session_folder(session_dir: str | Path) -> None:
    ...
    try:
        ...
        raise FileDeletionError(f"Could not delete session folder {path}: {exc}") from exc
